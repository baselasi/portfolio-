export default {
   personaleInfo:{
    name: 'Bassel',
    surename : 'ASSI',
    email:'bassel-assi@live.com',
    cellphone: 3201538330,
    address: 'via monte cengio, trieste'
   },
   aboutMe:{
    title:"FRONT-END DEVOLEOPER / PROCESS AND MATERIAL ENGINEER",
    profile:`Sono uno studente di magistrale in ingegneria di processo e materiali, con una grande passione e forte motivazione per diventare un sviluppatore fronte-end.
            competenza in javascript, css,html, react, Bootstrap e object-oriented-programing. Sono una
            persona seria con una grande capacità di adattamento e la voglia di imparare. Cerco la mia prima esperienza per fare partire la mia carriera.`
   },
   profile:'I am a process and materials engineer, i got my bachelor degree in industrial engineering, '+
            'then my a master degree in process and material engineering both from Trieste university. During my studies years i develop a interest in coding, and the intrest slowly become a passion, '+
            'so I decided to pursue a new carrier as programer. I taught myself how to code using the basic languages for front-end development (JAVASCRIPT ,HTML, CSS, BOOTSTRAP, REACT)'+
            '',
   studies: [
        {
            degree:'Laurea triennale in ingegneria industriale',
            schoole:'Università degli studi di Trieste',
            start:'2012',
            end:'2016'
        },
        {
            degree:'Corso magistrale in ingegneria dei processi e dei materiali',
            schoole:'Università degli studi di Trieste',
            start:'2017',
            end:'2023'
        }
   ],
   exprience:[
        {
            postion:'Porta pizza',
            company:'PIZZA PARINI',
            start:'2022',
            end:'oggi'
        },
        {
            postion:'Cameriere',
            company:'HOSTARIA MALCANTON',
            start:'2021',
            end:'2022'
        },
        {
            postion:'Venditore d’ auto',
            company:'REAUTO',
            start:'2021',
            end:'2021'
        }
   ],
   programnigSkills: [
        {
            skill:'JAVASCRIPT',
            competence: 75,
            logo: '../imgs/java-script.png'
        },
        {
            skill:'CSS',
            competence: 60,
            logo: '../imgs/css-3.png'
        },
        {
            skill:'HTML',
            competence: 75,
            logo: '../imgs/html (2).png'
        },
        {
            skill: 'REACT',
            competence: 60,
            logo: '../imgs/react.png'
        },
        {
            skill:'BOOTSTRAP',
            competence: 30,
            logo:'../imgs/bootstrap.png'
        },
        {
            skill:'SQL',
            competence: 20,
            logo: '../imgs/sql-server.png'
        },
    
   ],
   engineeringSkills:[
        {
            skill:'ASPEN-PLUS',
            competence:50,
            logo:'../imgs/aspenLogo.png'
        },
        {
            skill:'MATLAB',
            competence:50,
            logo:'../imgs/matalb.png'
        },
        {
            skill:'EDU-PACK',
            competence:70,
            logo:'../imgs/EDU-PACK.png'
        }
   ],
   softSkills:[
    {
        skill:'PROBLEM SOLVING',
        logo:'../imgs/problem-sovling.png'
    },
    {
        skill:'FLEXIBILITY',
        logo:'../imgs/flexibility.png'
    },
    {
        skill:'TEAM PLAYER',
        logo:'../imgs/team-player.png'
    },
    {
        skill:'RESPONSIVENESS',
        logo:'../imgs/responsevines.png'
    },
    {
        skill:'ORGANIZATION',
        logo:'../ings/organization.png'
    },

   ],
   languages : [
    {
        language:'ITALIANO',
        level: 90
    },
    {
        language:'INGLESE',
        level: 90
    },
    {
        language: 'ARABO',
        level:100
    }
   ]
   
}